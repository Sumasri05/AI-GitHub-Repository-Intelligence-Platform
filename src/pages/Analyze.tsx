...
import { saveRepositoryToStorage, saveHistoryToStorage } from "@/lib/storage";
import { analyzeRepoClientSide } from "@/lib/repoAnalyzer";
...
export default function Analyze() {
  ...
  const saveToHistory = async (data: Repository) => {
    saveRepositoryToStorage(data);
    saveHistoryToStorage(data, user?.id);
    if (!user) return;
    try {
      ...
      console.error("Failed to save history to Supabase (using storage fallback):", e);
    }
  };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (data && data.score !== undefined) {
        repo = data as Repository;
        isCached = (data as any)?._cached;
      } else {
        throw new Error("Invalid response from edge function");
      }
    } catch (e: any) {
      console.warn("Supabase edge function failed, running client-side repository analyzer fallback...", e);
      try {
        repo = await analyzeRepoClientSide(url.trim());
      } catch (clientErr: any) {
        toast({ title: "Analysis failed", description: clientErr.message || "Something went wrong", variant: "destructive" });
        setLoading(false);
        return;
      }
    }
    ...
    if (repo) {
      ...
      toast({
        ...
        description: `${repo.owner}/${repo.repo_name} scored ${Number(repo.score).toFixed(1)}/10${isCached ? " (cached)" : ""}`,
      });
    }
    setLoading(false);
  };

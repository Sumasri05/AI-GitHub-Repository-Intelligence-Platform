...
export default function Leaderboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data, error } = await supabase.from("repositories").select("*").order("score", { ascending: false });
        if (error || !data || data.length === 0) {
          const fallback = getStoredRepositories();
          setRepos(fallback.sort((a, b) => Number(b.score) - Number(a.score)));
        } else {

...
export default function Compare() {
  ...
  const analyzeSingle = async (url: string): Promise<Repository> => {
    try {
      const { data, error } = await supabase.functions.invoke("analyze-repo", { body: { url: url.trim() } });
      if (error) throw error;
      if (data && data.score !== undefined) return data as Repository;
      throw new Error("Invalid response");
    } catch {
      return await analyzeRepoClientSide(url.trim());
    }
  };
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      toast({ title: "Comparison failed", description: e.message || "Failed to analyze repositories for comparison", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
